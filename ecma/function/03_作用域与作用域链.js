...
/* 
  函数被定义时，生成的一个 js 内部的隐式属性 [[scopes]] 也是函数作用域链的存储容器
...
/* 
  ...
  2.作用域链中（栈结构）的第一项值指向的是 GO 全局执行期上下文（根据函数被定义时所处的环境如果是内部函数则第一项是外部函数的AO）
  3.当函数被调用在函数执行前（预编译阶段）作用域链顶端是自身的 AO 函数执行期上下文（这点也解释了为什么在函数外部无法访问函数内部的变量）
  4.将自身 AO 压入栈后第二项是全局的 GO，查找变量的规则从顶端依次向下查找
