/*
  ...
  2.重复声明会导致环境污染，可以使用立即执行函数解决全局环境变量污染
  3.变量声明提升，重点注意变量是提升至所处作用域的顶层，而不是全局作用域
*/ 
...
// es6 函数默认值声明的变量，存放在暂时性死区
function test3(x = y, y = 2){
...
function test4(num){
  {
    let num = 100; // let 声明变量产生块级作用域，不同作用域
    console.log(num); // 100
  }
  console.log(num); // 200
}
...
// test5(); // 函数声明提升到全局作用域
// console.log(test6) // 块级作用域没产生，var 声明变量提升至全局作用域

{
  let a = 1;
  // function a(){} // 报错，同一个块级作用域下重复声明
  {
    function a(){} // 不报错，函数声明仅提升至当前作用域下
  }
  console.log(a);
}
