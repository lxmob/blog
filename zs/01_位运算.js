/* 
  ...
  java中32位信息中可以表示的数字范围 (-2^31) - (2^31 - 1)

  java中 int 最大数为21e+在32位信息中表示的是 
  => [01111111111111111111111111111111]
  => 可以表示最大 int 数就是 (2^31 -1)
  ...
  => 不看符号位后面的数就等于 (~取反 + 1)
  => 0 是被算到非负领域中的所以在非负数中最大数是 (2^31 -1)
  => 在 java 中 int 最大数绝对值要比最小数绝对值少 1

  负数为什么要(~取反 + 1)呢
  => 在32位信息中加减乘除取余 并不是按照十进制进行运算的而是翻译成二进制位运算进行计算
  => 计算机在底层计算时按照位运算，通过一套逻辑解决正数与负数的运算
  => 在设计位运算中只走一套逻辑就想在实际执行的时候快一点
  => 例如 n 数负数就等于 = -n = (~n + 1)
...
console.log(1 << 2); // 2^2 => 4
console.log(1 << 3); // 2^3 => 8
